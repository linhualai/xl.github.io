...
titleField = field "pageTitle" $ \item -> do
  ...
  maybe (return "Tech Notes") (return . (++ " - Tech Notes")) title

customTitleField :: String -> Context String
customTitleField value = constField "pageTitle" $ value ++ " - Tech Notes"
...
gitTag key = field key $ \item -> do
  ...
  unsafeCompiler $ do
    ...
    return $ "<a href='https://github.com/xl/xl.github.io/commits/source/" ++ fp ++ "'>History</a>" ++
             "<span class='hash'>, <a href='https://github.com/xl/xl.github.io/commit/" ++ sha ++
